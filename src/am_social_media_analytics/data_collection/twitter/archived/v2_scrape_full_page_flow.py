...
def extract_articles_from_page(url: str):
    ...
    try:
        ...
        while total_written < stop_threshold:
            # STEP 0: Find all <article> elements that are direct children of <div class="css-175oi2r">
            articles = browser.find_elements(By.XPATH, '//div[contains(@class, "css-175oi2r")]/article')
            # STEP 0.0 Loop through all found articles
            for article in articles:
                try:
                    ## STEP 1: Extract the author name
                    try:
                    ...
                    ## STEP 2: Extract the ID of the post author
                ...
                if tweet_unique_key not in all_unique_keys:
                    logger.info("🆔 Current tweet with unique key: " + tweet_unique_key + " is not present. It will be added.")
            ...
            pause_time = random.uniform(25, 40)
        ...
        if len(articles_set) > 0:
            current_datetime = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = f"articles_final_due_to_exception_{total_written + 1}_{current_datetime}.csv"
            with open(output_file, mode="w", encoding="utf-8", newline="") as file:
                writer = csv.writer(file)
                writer.writerow([
                    "Author Name",
